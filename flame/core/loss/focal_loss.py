...
class FocalLoss(loss.LossBase):
    ...
    def _xyxy2xywh(self, anchors, boxes):
        ...
        converted_boxes = torch.stack([ctr_x_boxes, ctr_y_boxes,
                                       width_boxes, height_boxes], dim=1)
        ...
        converted_anchors = torch.stack([ctr_x_anchors, ctr_y_anchors,
                                         width_anchors, height_anchors], dim=1)
    ...
    def _smooth_l1_loss(self, input: torch.Tensor = None, target: torch.Tensor = None, reduction: str = 'none', beta: float = 1.):
    ...
    def _focal_loss(self, cls_pred: torch.Tensor = None, cls_target: torch.Tensor = None, reduction: str = 'none'):
    ...
    def forward(self, cls_preds, reg_preds, anchors, targets):
        ...
        average_loss = cls_loss.mean() + self.lamda * reg_loss.mean()
