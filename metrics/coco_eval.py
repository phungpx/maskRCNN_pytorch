...
class COCOEvaluator(MetricBase):
    def __init__(
        ...
        if all(
            [
                annotation_type in ["segm", "bbox", "keypoints"]
    ...
    def reset(self):
        # List[{'image_id': ..., 'category_id': ..., 'score': ..., 'bbox': ...}]
    ...
    def update(self, output):
        ...
        for pred, target, image_info in zip(preds, targets, image_infos):
            ...
            scale = (
                max(image_info[1]) / max(self.image_size)
                if self.image_size is not None
                else 1.0
            )  # deal with input sample is paded to square (bottom-right)

            pred_boxes[:, [2, 3]] -= pred_boxes[
                :, [0, 1]
            ]  # convert x1, y1, x2, y2 to x1, y1, w, h
            pred_boxes = (
                (pred_boxes * scale).astype(np.int32).tolist()
            )  # scale boxes to orginal size.
            ...
            if self.annotation_file is None:
                ...
                true_boxes[:, [2, 3]] -= true_boxes[
                    :, [0, 1]
                ]  # convert x1, y1, x2, y2 to x1, y1, w, h
                true_boxes = (
                    (true_boxes * scale).astype(np.int32).tolist()
                )  # scale boxes to orginal size.

                # create ground truth in COCO format if has no COCO ground truth file.
                for box, label, area, mask in zip(
                    true_boxes, true_labels, true_areas, true_masks
                ):
                    if label == -1:
                        continue
    ...
    def compute(self):
        if not len(self.detections):
            raise Exception(
                "the model does not provide any valid output,\
