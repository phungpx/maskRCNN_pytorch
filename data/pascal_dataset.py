...
class PascalDataset(Dataset):
    def __init__(
        ...
        image_paths = natsorted(
            list(Path(image_dir).glob(f"*{image_extent}")), key=lambda x: str(x.stem)
        )
        label_paths = natsorted(
            list(Path(label_dir).glob(f"*{label_extent}")), key=lambda x: str(x.stem)
        )
        voc2007_pairs = [
            [image, label]
        ...
        # VOC2012
        image_dir, label_dir, txt_path = (
            Path(VOC2012["image_dir"]),
            Path(VOC2012["label_dir"]),
            Path(VOC2012["txt_path"]),
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.image_size is not None:
            ...
            image, bboxes = iaa.Resize(size=self.imsize)(
        ...
        sample = (sample.float().div(255.0) - self.mean) / self.std
