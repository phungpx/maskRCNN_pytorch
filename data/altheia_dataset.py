...
class AltheiaDataset(Dataset):
            ...
            for region in regions:
                ...
                if w >= x2 > x1 and h >= y2 > y1:
                    ...
                    annots.append(
                        {
                            "mask": cv2.fillPoly(
                                img=mask, pts=np.int32([points]), color=1
                            ),
                            "label": label,
                            "box": (x1, y1, x2, y2),
                        }
    ...
    def __getitem__(
        self, idx: int
    ) -> Tuple[torch.Tensor, Dict, Tuple[str, Tuple[int, int]]]:
        ...
        masks = [
            SegmentationMapsOnImage(arr=mask, shape=image.shape[:2]) for mask in masks
        ]
        ...
        for transform in random.sample(
            ...
            fixed_transform = (
                transform.to_deterministic()
            )  # fix random setting in each transforms
